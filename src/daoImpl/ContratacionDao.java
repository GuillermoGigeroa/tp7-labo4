...
public class ContratacionDao implements AccesoDatosContratacion {
	...
	public int agregarContratacion(Contratacion contrato) {
		String query = "insert into contratacion (nombreUsuario, idSeguro, costoContracion) values ('"
				+ contrato.getNombreUsuario() + "'," + contrato.getIdSeguro() + "," + contrato.getCostoContratacion()
				+ "')";
	...
	public Boolean modificarContatacion(int _idContratacion, String _nombreUsuario, int _idSeguro,
			double _costoContratacion) {

		boolean hecho = false;

		try {
			Statement st = conectar.conexion().createStatement();
			String query = "Update contratacion set nombreUsuario = '" + _nombreUsuario + "', idSeguro = " + _idSeguro
					+ " , costoContratacion = " + _costoContratacion + " where idContratacion = " + _idContratacion
					+ "";
